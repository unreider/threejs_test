...
const Door1 = (props: ThreeElements["mesh"]) => {
    ...
    const { scene: door3 } = useGLTF("/models/doors/door3/1.MANCHESTER_Door.glb");
    ...
    useEffect(() => {
        if (doorRef.current && handle3 && handle4 && frame3) {
            ...
            doorRef.current.add(frame3);
        }
    }, [door3, handle3, handle4, frame3]);
    ...
    return (
        <group
            ...
            scale={scale}
        >
            <primitive object={door3} ref={doorRef} />
        </group>
    );
};
